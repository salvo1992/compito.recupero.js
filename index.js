...
// Inizialmente, tieni il codice che genera l'HTML per le attività non completate
let incompleteTasksHTML = "";
for (let i = 0; i < tasks.length; i++) {
  const task = tasks[i];
  if (!task.completed) {
    incompleteTasksHTML += createTaskElement(task);
  }
}
taskContainer.innerHTML = incompleteTasksHTML;


// Funzione per filtrare le task e visualizzarle
// Funzione per filtrare le task e visualizzarle
function filterTasks() {
    ...
    for (let i = 0; i < tasks.length; i++) {
      const task = tasks[i];
      if (task.todo.toLowerCase().includes(keyword)) {
        filteredTasks.push(task);
      }
    }
  
    displayTasks(filteredTasks); // Visualizza le task filtrate
  }
